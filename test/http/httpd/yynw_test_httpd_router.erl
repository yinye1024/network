...
priv_route_request(Req,DocRoot)->
  ...
  case Path0 of
    "file"->
      ...
      ?OK;
    "auth"->
      yynw_test_httpd_auth_handler:handle(Req,LessPath),
      ?OK;
    _->
      case yynw_test_httpd_auth_handler:check_auth(Req) of
        {?TRUE,UserId} ->
          Resp = priv_route_bs(Path0,{UserId,Method,Req,LessPath}),
          yynw_http_utils:resp_ok(Resp,Req);
        {?FALSE,_} ->
          Resp = yynw_test_resp_restful:new_fail("授权失败，请重新登陆"),
          yynw_http_utils:resp_ok(yynw_test_httpd_auth_handler:set_auth_fail(Resp),Req)
      end,
      ?OK
  end,
  ?OK.
...
priv_route_bs("test",{_UserId,Method,Req,LessPath})->
  Resp =  yynw_test_httpd_bs_handler:handle(Method,Req,LessPath),
  Resp;
priv_route_bs(_Default,{_UserId,_Method,_Req,_LessPath})->
  ?LOG_INFO({"请求不存在",_Req}),
  Resp =  yynw_test_resp_restful:new_fail("请求不存在"),
  Resp.
