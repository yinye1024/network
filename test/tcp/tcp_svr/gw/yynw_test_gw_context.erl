...
-export([new/0]).
-export([on_active/2,on_new_tpl/2,on_reconnect/2,on_login/2,on_time_out/2]).
-export([get_ctype/1,get_tplId/1,get_userId/1,get_svrId/1]).
...
get_tplId(SelfMap)->
  yyu_map:get_value(tplId, SelfMap).
get_ctype(SelfMap) ->
...
on_new_tpl({NewTplId},SelfMap)->
  SelfMap#{
    tplId=>NewTplId
  }.
on_reconnect({SvrId,TplId},SelfMap)->
  SelfMap#{
    svrId =>SvrId,
    tplId=>TplId
  }.

on_login({_Platform,_MachineInfo,SvrId,_UserId,_UserName,TplId},SelfMap)->
  SelfMap#{
    svrId =>SvrId,
    tplId=>TplId
  }.
