...
-export([new_pojo/1]).
-export([get_id/1, get_client_mid/1, set_client_mid/2, get_bs_gen/1, set_bs_gen/2]).
...
new_pojo(DataId)->
  #{
    ...
    bs_gen => ?NOT_SET,
    context => yynw_test_gw_context:new(),
...
get_bs_gen(SelfMap) ->
  yyu_map:get_value(tpl_gen, SelfMap).

set_bs_gen(Value, SelfMap) ->
  yyu_map:put_value(tpl_gen, Value, SelfMap).
