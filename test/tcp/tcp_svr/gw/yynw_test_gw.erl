...
handle_active_pack(_PackData = <<UserId:?UserIdBitSize,Ticket:?TicketBitSize>>)->
  ...
  yynw_test_gw_pc_mgr:init(),
  ContextItem = yynw_test_gw_pc_mgr:get_context(),
  case priv_check_auth(UserId,Ticket) of
    ?TRUE ->
      ContextItem_1 = yynw_test_gw_context:on_active({UserId,Ticket},ContextItem),
      yynw_test_gw_pc_mgr:set_context(ContextItem_1),
      RolePid = gs_yynw_test_role_mgr:new_child({UserId,self()}),
      priv_set_self_monitor_by(RolePid),
      Success = 1,
      yynw_test_gw_helper:send_connect_active_s2c(Success),
      ?OK;
    ?FALSE ->
      Fail = 0,
      yynw_test_gw_helper:send_connect_active_s2c(Fail),
      yynw_test_gw_helper:cast_stop(?Logout_Active_Auth_Fail),
      ?OK
  end,
  ?OK.
priv_set_self_monitor_by(RolePid)->
  yyu_pid:set_self_monitor_by(RolePid),
  yynw_test_gw_pc_mgr:set_bs_gen(RolePid),
  ?OK.
...
route_c2s(BodyPack)->
  ...
  RoleGen = yynw_test_gw_pc_mgr:get_bs_gen(),
  gs_yynw_test_role_mgr:handle_msg(RoleGen,{MsgId, C2SId,C2SData}),
  ?OK.
...
check_heartbeat()->
  case yynw_test_gw_pc_mgr:is_max_heartbeat_time_out() of
    ?TRUE ->
      yynw_test_gw_helper:cast_stop(?Logout_Heartbeat_Timeout_Reach_Max),
      ?OK;
    ?FALSE ->
      yynw_test_gw_pc_mgr:check_heartbeat(),
      ?OK
  end,
  ?OK.
%%======================= 心跳 相关 结束 ===================================================
