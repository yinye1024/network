...
init({RoleId,TcpGen} = _GenArgs)->
  yynw_test_role_mgr:init({RoleId,TcpGen}),
  yynw_test_role_gen_mgr:reg(RoleId,self()),
  ?OK.
...
terminate(RoleId)->
  yynw_test_role_gen_mgr:un_reg(RoleId),
  ?OK.

send_msg({MsgId,C2SId,BinData})->
  ?LOG_INFO({"role send msg:",{MsgId,C2SId,BinData}}),
  RoleGen = yynw_test_role_mgr:get_tcp_gen(),
  yynw_tcp_gw_api:send(RoleGen,{MsgId,C2SId,BinData}),
  ?OK.
remove_tcpGen({})->
  yynw_test_role_mgr:switch_tcp_gen(?NOT_SET),
  ?OK.

switch_tcpGen({TcpGen})->
  yynw_test_role_mgr:switch_tcp_gen(TcpGen),
  ?OK.
